...
module.exports={
	...
	"tonesInOctave": ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"]
}
...
var assignKeyValue = function assignKeyValue(o, k, v) {
	return (o[k] = v) && o;
};

var createCanvas = _fn2.default.compose(function () {
	return _dom2.default.create('canvas', {
		width: 24,
		height: 24
	});
}, function (canvas) {
...
var createIcon = function createIcon(ctx, code) {
	ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
	ctx.fillText(code, 12, 12);
	return ctx.canvas.toDataURL('image/png');
};

var prepIcons = _fn2.default.compose(function (list) {
	return { ctx: createCanvas(), list: list };
}, function (_ref) {
	var ctx = _ref.ctx,
	    list = _ref.list;
	return Object.keys(list).reduce(function (o, key) {
		return assignKeyValue(o, key, createIcon(ctx, list[key]));
	}, {});
});

var setCursor = function setCursor(el, dataURL) {
...
},{"./dom":4,"./fn":5}],4:[function(require,module,exports){
'use strict';
...
var find = function find(el, query) {
...
var findOne = _fn2.default.compose(find, function (elList) {
	return elList.length === 0 && elList.pop() || elList;
});

// el.class#id
var create = function create(code, attr) {
	var id = code.match(/(#[a-zA-Z0-9\-]+)/ig);
	id = id && id.pop().replace('#', '');
	var classes = code.match(/(\.[a-zA-Z0-9\-]+)/ig);
	classes = classes && classes.map(function (cls) {
		return cls.replace('.', '');
	}) || [];
	var tagName = code.replace('#' + id, '').replace('.' + classes.join('.'), '');
	var el = Object.assign(document.createElement(tagName), attr);
...
var singleOn = function singleOn(el, eventName, selector, cb) {
	return el instanceof HTMLElement || el === document ? el.addEventListener(eventName, function (ev) {
		...
		if (typeof selector === 'string' && typeof cb !== 'undefined') {
			var selectedList = listToArray(el.querySelectorAll(selector));
			var evPath = ev.path || [ev.target];
			var selectedEl = evPath.reduce(function (sel, el) {
...
var on = function on(el, eventName, selector, cb) {
	return el instanceof Array ? el.forEach(function (el) {
		return singleOn(el, eventName, selector, cb);
...
},{"./fn":5}],5:[function(require,module,exports){
'use strict';
...
var compose = function compose() {
	...
	return function () {
		...
		return fList.reduce(function (r, f) {
			return r instanceof Array && f.apply(null, r) || f(r);
		}, args);
	};
};
...
var _jsl = require('./jsl');

var _jsl2 = _interopRequireDefault(_jsl);

var _dom = require('./iblokz/dom');

var _dom2 = _interopRequireDefault(_dom);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

window.domUtil = _dom2.default;
...
window.loadApp = function () {
	var studio = new _jsl2.default.gui.Studio('body');
...
var Counter = function () {
	function Counter(intervals, range, direction, position) {
	...
	_createClass(Counter, [{
		...
		value: function setPosition(position) {
			this.position = position && position.slice() || this.direction === 1 && this.range[0].slice() || this.range[1].slice();
			return this;
		}
	}, {
		key: 'setPositionStart',
		value: function setPositionStart() {
			this.position = this.direction === 1 ? this.range[0].slice() : this.range[1].slice();
			return this;
		}
	}, {
		key: 'setPositionEnd',
		value: function setPositionEnd() {
			this.position = this.direction === 1 ? this.range[1].slice() : this.range[0].slice();
			return this;
		}
	}, {
		key: 'fromSteps',
		value: function fromSteps(steps) {
			this.setPosition();
			this.iterate(steps);
			return this;
		}
		...
		key: 'merge',
		value: function merge(counter, direction) {
...
var Canvas = function () {
	function Canvas(dom) {
		...
		this.dom = dom instanceof HTMLElement ? dom : _dom2.default.findOne(dom);
	...
	_createClass(Canvas, [{
		...
		key: 'line',
		value: function line(start, finish, stroke, dash) {
			this.ctx.beginPath();
		...
		key: 'rect',
		value: function rect(_rect, background, stroke, dash) {
			this.ctx.beginPath();
			this.ctx.rect(_rect.x, _rect.y, _rect.width, _rect.height);
			if (background) {
...
},{"../../iblokz/dom":4,"./rect":11,"./vector2":12}],10:[function(require,module,exports){
'use strict';
...
var _counter = require('../etc/counter');
...
var Grid = function (_Canvas) {
	_inherits(Grid, _Canvas);

	function Grid(dom, conf) {
		_classCallCheck(this, Grid);

		var _this = _possibleConstructorReturn(this, (Grid.__proto__ || Object.getPrototypeOf(Grid)).call(this, dom));

		_this.conf = conf;
		var range = conf.range;
		var position = conf.position;

		_this.step = new _vector2.default().copy(conf.step);

		_this.counter = {
			x: new _counter2.default(range.x.length, [range.x.start, range.x.end], range.x.direction, position.x),
			y: new _counter2.default(range.y.length, [range.y.start, range.y.end], range.y.direction, position.y)
		};

		_this.playHead = _this.counter.x.clone();

		_this.offset = new _vector2.default(_this.counter.x.toSteps() * conf.step.x, _this.counter.y.toSteps() * conf.step.y);

		_this.range = new _rect2.default();

		_this.range.setStart({
			x: _this.counter.x.clone().setPositionStart().toSteps() * conf.step.x,
			y: _this.counter.y.clone().setPositionStart().toSteps() * conf.step.y
		});

		_this.range.setEnd({
			x: _this.counter.x.clone().setPositionEnd().toSteps() * conf.step.x,
			y: _this.counter.y.clone().setPositionEnd().toSteps() * conf.step.y
		});
	...
	_createClass(Grid, [{
		...
		value: function setConf(conf) {
			this.conf = conf;
			if (conf.position) this.position = conf.position;
		}
	}, {
		key: 'init',
		value: function init() {
		...
		value: function refresh() {
			...
			var isNextStep = function isNextStep(index, position, length) {
				var isNextStep = true;
   	...
   	this.visible = this.calculateVisible();
   		...
   		y: grid.multiSubtract(grid.position.y, grid.visible.y, conf.range.y.length)
			...
			for (var _yStep = 0; _yStep < sizeVector[1]; _yStep += step.y) {
				...
				this.rect(new _rect2.default(0, _yStep, step.x, _yStep + step.y), pianoBGColor, '#000');

				this.rect(new _rect2.default(step.x, _yStep, sizeVector[0], _yStep + step.y), backgroundColor);

				if (sections.y.octave.pattern.labels[yCounter.position[1]] !== '') {
					ctx.font = '12px Arial';
					ctx.fillStyle = pianoFGColor;
					ctx.fillText(sections.y.octave.pattern.labels[yCounter.position[1]] + yCounter.position[0], 2, _yStep - 6 + 18);
				}

				var _borderColor = '';
				for (var _index = yCounter.position.length - 1; _index >= 0; _index--) {
					if (_index === yCounter.position.length - 1) {
						if (parseInt((yCounter.position[_index] + 1) / range.y.length[_index], 10) === (yCounter.position[_index] + 1) / range.y.length[_index]) {
							_borderColor = colors[sections.y[conf.order.y[_index]].border];
						}
					} else if (isNextStep(_index, yCounter.position, range.y.length)) {
						_borderColor = colors[sections.y[conf.order.y[_index]].border];
					}
				}

				this.line([step.x, _yStep], [sizeVector[0], _yStep], _borderColor);
			...
			for (var xStep = step.x; xStep < sizeVector[0]; xStep += step.x) {
				...
				for (var index = xCounter.position.length - 1; index >= 0; index--) {
					if (index === xCounter.position.length - 1) {
						if (parseInt((xCounter.position[index] + 1) / range.x.length[index], 10) === (xCounter.position[index] + 1) / range.x.length[index]) {
			...
			for (var yStep = 0; yStep < sizeVector[1]; yStep += step.y) {
				this.elements.forEach(function (element, elementIndex) {
					if (_lodash2.default.isEqual(element.counter.y.position, yCounter.position)) {
						...
						if (elXPos >= step.x && elXPos <= sizeVector[0] || elXPos + elWidth > step.x && elXPos + elWidth <= sizeVector[0] || elXPos < step.x && elXPos + elWidth > sizeVector[0]) {
							if (elXPos - step.x < 0) {
							...
							grid.rect(new _rect2.default(elXPos + 1, yStep + 1, elWidth - 2, step.y - 2), elColor);

							grid.hitAreas.push({
								rect: new _rect2.default().copy({
									x: elXPos + 1,
									y: yStep + 1,
									width: elWidth - 2,
									height: step.y - 2
...
},{"../etc/counter":7,"./canvas":9,"./rect":11,"./vector2":12,"lodash":1}],11:[function(require,module,exports){
'use strict';
...
var Rect = function () {
	...
	_createClass(Rect, [{
		...
		value: function getStart() {
			var start = new _vector2.default();
			start.x = this.x;
			start.y = this.y;
			return start;
		}
		...
		key: 'fromVectors',
		value: function fromVectors(a, b) {
			if (a.x <= b.x) {
				this.x = a.x;
				this.width = b.x - a.x;
			} else {
		...
		key: 'containsVector',
		value: function containsVector(v) {
		...
		value: function containsRect(rect) {
			var start = this.getStart();
			var end = this.getEnd();
			if (start.x <= rect.x && rect.x + rect.width <= end.x && start.y <= rect.y && rect.y + rect.height <= end.y) {
				return true;
			}
			return false;
		}
		...
		value: function contains(obj) {
			...
			if (obj instanceof _vector2.default) {
...
},{"./vector2":12}],12:[function(require,module,exports){
'use strict';
...
var Vector2 = function () {
	...
	_createClass(Vector2, [{
		...
		key: 'fromArray',
		value: function fromArray(coordArr) {
		...
		key: 'convertToThreeJSVector2',
		value: function convertToThreeJSVector2(THREE) {
...
var _signal = require('../etc/signal');
...
var View = function () {
	function View(dom, config) {
		...
		this.interaction = {
			...
			start: new _vector2.default(0, 0),
			last: new _vector2.default(0, 0)
		};

		this.signals = {
			create: new _signal2.default()
		};

		this.layers = {
			grid: new _grid2.default(_dom2.default.findOne(this.dom, 'canvas.grid'), this.config),
			elements: new _canvas2.default(_dom2.default.findOne(this.dom, 'canvas.elements')),
			interaction: new _canvas2.default(_dom2.default.findOne(this.dom, 'canvas.interaction'))
		};
	}
	...
	_createClass(View, [{
		...
		value: function init() {
			...
			_dom2.default.on(this.dom, 'mousewheel', function (event) {
				var panVector = new _vector2.default(0, 0);
			...
			_dom2.default.on(this.dom, 'mousedown', function (event) {
			...
			_dom2.default.on(this.dom, 'mousemove', function (event) {
				if (['mousedown', 'mousemove'].indexOf(view.interaction.status) > -1) {
					view.interaction.status = 'mousemove';
					var currentPos = new _vector2.default(event.offsetX, event.offsetY);
					...
					switch (view.interaction.action) {
						...
						case 'selecting':
							{
								var selectionRect = new _rect2.default().fromVectors(view.interaction.start, currentPos);
						...
						case 'moving':
							{
								...
								if (xSteps !== 0 || ySteps !== 0) {
									...
									grid.refresh();
									view.interaction.last.add(new _vector2.default(parseInt(changeSinceLast.x / grid.conf.step.x, 10) * grid.conf.step.x, parseInt(changeSinceLast.y / grid.conf.step.y, 10) * grid.conf.step.y));
								}
								break;
							}
						case 'resizing':
							{
								var _changeSinceLast = currentPos.clone().sub(view.interaction.last);
								var _xSteps = parseInt(Math.abs(_changeSinceLast.x) / grid.conf.step.x, 10);
								if (_xSteps !== 0) {
									grid.selection.forEach(function (elementIndex) {
										grid.elements[elementIndex].length.iterate(_xSteps, _changeSinceLast.x / Math.abs(_changeSinceLast.x));
										grid.elements[elementIndex].update();
									});
									grid.refresh();
									view.interaction.last.add(new _vector2.default(parseInt(_changeSinceLast.x / grid.conf.step.x, 10) * grid.conf.step.x, parseInt(_changeSinceLast.y / grid.conf.step.y, 10) * grid.conf.step.y));
								}
								break;
							}
						default:
			...
			_dom2.default.on(this.dom, 'mouseup', function (event) {
			...
			_dom2.default.on(document, 'keyup', function (event) {
...
},{"../../iblokz/dom":4,"../etc/signal":8,"./canvas":9,"./grid":10,"./rect":11,"./vector2":12}],14:[function(require,module,exports){
'use strict';
...
},{"../../iblokz/dom":4}],15:[function(require,module,exports){
'use strict';
...
var _pianoRoll = require('./piano-roll');
...
exports.default = {
	...
	PianoRoll: _pianoRoll2.default,
	Sequencer: _sequencer2.default,
	Studio: _studio2.default
};

},{"./editor":14,"./piano-roll":16,"./sequencer":17,"./studio":18}],16:[function(require,module,exports){
'use strict';
...
var _measure = require('../../config/measure');

var _dom = require('../../iblokz/dom');

var _dom2 = _interopRequireDefault(_dom);

var _cursor = require('../../iblokz/cursor');

var _cursor2 = _interopRequireDefault(_cursor);

var _view = require('../gfx/view');
...
var PianoRoll = function (_Editor) {
	_inherits(PianoRoll, _Editor);

	function PianoRoll(dom) {
		_classCallCheck(this, PianoRoll);

		var _this = _possibleConstructorReturn(this, (PianoRoll.__proto__ || Object.getPrototypeOf(PianoRoll)).call(this, dom));

		_this.viewConfig = {
			colors: ['#000', '#111', '#333', '#444', '#555', '#777'],
			step: {
				x: 38,
				y: 14
			},
				...
				y: {
					...
					octave: {
						...
						pattern: {
							...
							backgrounds: [1, 0, 1, 0, 1, 1, 0, 1, 0, 1, 0, 1],
							color: 4
						}
					}
				}
		...
		_this.view = new _view2.default(_dom2.default.findOne(_this.dom, '.view'), _this.viewConfig);

		_this.track = {};

		_this.timeToPosition = function (time) {
			var measure = parseInt(time / _measure.intervals.measure, 10);
			time -= measure * _measure.intervals.measure;
			var beat = parseInt(time / _measure.intervals.beat, 10);
			time -= beat * _measure.intervals.beat;
			var tick = parseInt(time / _measure.intervals.tick, 10);
			time -= tick * _measure.intervals.tick;
			return [measure, beat, tick];
		};

		_this.positionToTime = function (position) {
			var time = 0;
			time += position[0] * _measure.intervals.measure;
			time += position[1] * _measure.intervals.beat;
			time += position[2] * _measure.intervals.tick;
			return time;
		};
		return _this;
	}
	...
	_createClass(PianoRoll, [{
		key: 'init',
		value: function init() {
			_get(PianoRoll.prototype.__proto__ || Object.getPrototypeOf(PianoRoll.prototype), 'init', this).call(this);
			this.view.init();
			...
			var icons = _cursor2.default.prepIcons({
				edit: '\uF040',
				delete: '\uF12D'
			});

			_dom2.default.on(this.dom, 'click', '[class*=\'-option\']', function (ev) {
				var optionParam = _dom2.default.get(ev.target, 'data-option-param');
				var optionValue = _dom2.default.get(ev.target, 'data-option-value');
				_dom2.default.find(pianoRoll.dom, '.' + optionParam + '-option').map(function (el) {
					return el.classList.remove('selected');
				});
				ev.target.classList.add('selected');

				if (icons[optionValue]) {
					_cursor2.default.setCursor(view.dom, icons[optionValue]);
				} else {
		...
		value: function create() {
			...
			return function (counter) {
				...
				event.note = _measure.tonesInOctave[counter.y.position[1]];
				event.octave = counter.y.position[0];
				event.duration = _measure.durations.eighth;
				...
				if (event.type === 'noteon') {
					var element = {
						position: {
							...
							y: [event.octave || 4, _measure.tonesInOctave.indexOf(event.note)]
						},
						...
						update: function update() {
							...
							pianoRoll.track.events[eventIndex].note = _measure.tonesInOctave[this.counter.y.position[1]];
...
},{"../../config/measure":2,"../../iblokz/cursor":3,"../../iblokz/dom":4,"../gfx/view":13,"./editor":14}],17:[function(require,module,exports){
'use strict';
...
var Sequencer = function (_Editor) {
	_inherits(Sequencer, _Editor);

	function Sequencer(dom, context) {
		_classCallCheck(this, Sequencer);

		var _this = _possibleConstructorReturn(this, (Sequencer.__proto__ || Object.getPrototypeOf(Sequencer)).call(this, dom, context));
	...
	_createClass(Sequencer, [{
		...
		value: function init() {
			...
			_get(Sequencer.prototype.__proto__ || Object.getPrototypeOf(Sequencer.prototype), 'init', this).call(this, this);
			...
			_dom2.default.on(this.dom, 'mousedown', '.bar', function (ev) {
				mousedown = true;
				lastBar = ev.target;
			...
			_dom2.default.on(this.dom, 'mousemove', '.bar', function (ev) {
				if (mousedown === true && lastBar !== _this2) {
					lastBar = _this2;
					ev.target.classList.toggle('selected');
					sequencer.refresh();
				}
			...
			_dom2.default.on(document, 'mouseup', function () {
				mousedown = false;
				lastBar = false;
			});

			_dom2.default.on(this.dom, 'change', '#bar-count', function (ev) {
		...
		key: 'redraw',
		value: function redraw() {
			var channelsEl = _dom2.default.findOne(this.dom, '.channels');
			_dom2.default.clear(channelsEl);
			if (this.track && this.track.channels) {
				this.track.channels.forEach(function (chan, i) {
					var chanEl = _dom2.default.create('div.channel#channel' + i);
					chanEl.appendChild(_dom2.default.create('div.channel-name', {
						textContent: chan.name
					}));
					chanEl.appendChild(_dom2.default.create('input.channel-note', {
						value: chan.note
					}));

					var barsEl = _dom2.default.create('div.bars');

					var patternIndex = 0;
					for (var bar = 0; bar < _dom2.default.findOne('#bar-count').value; bar++) {
						var barEl = _dom2.default.create('div.bar#channel' + i + '-bar' + bar);
						if (chan.pattern) {
		...
		value: function refresh() {
			...
			if (sequencer.track) {
				...
				_dom2.default.find(this.dom, '.bars').forEach(function (barsEl, chanI) {
					sequencer.track.channels[chanI].pattern = _dom2.default.find(barsEl, '.bar').map(function (barEl, barI) {
						return barEl.classList.contains('selected') ? 1 : 0;
					});
				});
			}
		}
		...
		value: function link(track) {
			this.track = track;
			_dom2.default.findOne(this.dom, '#bar-count').value = track.bars;
			this.redraw();
			this.refresh();
		}
	}, {
		key: 'tick',
		value: function tick(barIndex) {
			_dom2.default.find(this.dom, '.bars').forEach(function (chanEl) {
				return _dom2.default.find(chanEl, '.bar').forEach(function (el, i) {
					return i === barIndex ? !el.classList.contains('current') && el.classList.add('current') : el.classList.remove('current');
				});
			});
		}
	}, {
...
},{"../../iblokz/dom":4,"./editor":14}],18:[function(require,module,exports){
'use strict';
...
var _sampler = require('../instr/sampler');

var _sampler2 = _interopRequireDefault(_sampler);

var _basicSynth = require('../instr/basic-synth');

var _basicSynth2 = _interopRequireDefault(_basicSynth);
...
var Studio = function (_Editor) {
	_inherits(Studio, _Editor);

	function Studio(dom, context) {
		_classCallCheck(this, Studio);

		var _this = _possibleConstructorReturn(this, (Studio.__proto__ || Object.getPrototypeOf(Studio)).call(this, dom, context));

		_this.actx = new AudioContext();
		...
		_this.kit = {
			Sampler: {
				B0: new _sampler2.default(_this.actx, 'samples/kick01.ogg'),
				Bb1: new _sampler2.default(_this.actx, 'samples/hihat_opened02.ogg'),
				D1: new _sampler2.default(_this.actx, 'samples/snare01.ogg'),
				Eb1: new _sampler2.default(_this.actx, 'samples/clap01.ogg')
			},
			Piano: new _basicSynth2.default(_this.actx, 'C5')
		};

		_this.sequencer = new _sequencer2.default(_dom2.default.findOne(_this.dom, '.sequencer'));
		_this.pianoRoll = new _pianoRoll2.default(_dom2.default.findOne(_this.dom, '.piano-roll'));
		return _this;
	}
	...
	_createClass(Studio, [{
		...
		value: function tick() {
			var studio = this;
			if (this.tickIndex < this.song.bars - 1) this.tickIndex++;else this.tickIndex = 0;
			...
			this.song.arrangement.forEach(function (item) {
				...
				if (studio.tickIndex >= item.start && studio.tickIndex < item.start + track.bars * item.repeat) {
					var trackTick = studio.tickIndex - item.start - parseInt(studio.tickIndex / track.bars, 10) * track.bars;
       ...
       */
		...
		value: function load(data) {
			...
			_dom2.default.findOne('#bpm').value = this.song.bpm;
		...
		value: function init() {
			...
			_dom2.default.on(_dom2.default.findOne(this.dom, '#play'), 'click', function (ev) {
				if (playLoop) {
					clearInterval(playLoop);
					playLoop = false;
				} else {
					var bpm = parseInt(_dom2.default.findOne('#bpm').value, 10);
     	...
     	song.tracks[1].events.forEach(function(event) {
			...
			_dom2.default.on(_dom2.default.findOne(this.dom, '#stop'), 'click', function (ev) {
				if (playLoop) {
					clearInterval(playLoop);
				}
				playLoop = false;
				_dom2.default.find(_this2.dom, '.bars .bar').forEach(function (barEl) {
					barEl.classList.remove('current');
				});

				studio.tickIndex = -1;

				_dom2.default.findOne('#play').classList.remove('active');
			});

			_dom2.default.on(_dom2.default.findOne(this.dom, '#bpm'), 'change', function (ev) {
			...
			_dom2.default.on(_dom2.default.findOne(this.dom, '#save'), 'click', function (ev) {
				var blob = new Blob([JSON.stringify(song)], { type: 'text/plain;charset=utf-8' });
				window.saveAs(blob, 'song.json');
			});

			_dom2.default.on(_dom2.default.findOne(this.dom, '#load'), 'click', function (ev) {
				_dom2.default.findOne('#load-file').click();
			});

			_dom2.default.on(_dom2.default.findOne(this.dom, '#load-file'), 'change', function (ev) {
				var receivedText = function receivedText(ev) {
			...
			_dom2.default.on(_dom2.default.findOne(this.dom, '#clear'), 'click', function (ev) {
...
},{"../../iblokz/dom":4,"../instr/basic-synth":20,"../instr/sampler":21,"./editor":14,"./piano-roll":16,"./sequencer":17}],19:[function(require,module,exports){
"use strict";
...
},{"./gui":15}],20:[function(require,module,exports){
'use strict';
...
var BasicSynth = function () {
	function BasicSynth(context, note) {
		_classCallCheck(this, BasicSynth);
	...
	_createClass(BasicSynth, [{
		key: 'noteToFrequency',
		value: function noteToFrequency(note) {
   ...
   */
		...
		key: 'trigger',
		value: function trigger(time, duration, note) {
			note = note || this.note;
...
var Sampler = function () {
	...
	_createClass(Sampler, [{
		...
		key: 'trigger',
		value: function trigger(start, end) {
			this.setup();
